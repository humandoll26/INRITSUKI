...
このリポジトリには、VOICEVOX 本体や音声ライブラリは含めないでください。  
このツールは VOICEVOX と連携するフロントエンドツールとして公開し、VOICEVOX 自体は利用者が別途インストールする形を前提にしてください。
...
README や公開ページには、少なくとも次のような表記を入れるのが無難です。

`このツールは VOICEVOX を利用しています。`

## ライセンスまわりの補足

この README は法的助言ではありません。公開前には、VOICEVOX 本体の最新規約と、使用するキャラクターごとの利用条件を必ず確認してください。
